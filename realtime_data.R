...
#plots
ggplot(aggrData,aes(run,raw_fb))+stat_summary(fun.y = mean,geom = 'point')+stat_summary(fun.y = mean,geom = 'line', aes(group=1))+stat_summary(fun.data = mean_cl_boot,geom = 'errorbar', width=0.4)+scale_x_continuous(breaks = c(1:6))+ggtitle('Raw feedback value during training')
